...
pub enum MessageAuthenticity {
    ...
    ///
    /// The specified [`NodeId`] will be used as the author of all published messages. The sequence
    /// number will be randomized.
    Author(NodeId),
    /// Message signing is disabled.
    ///
    /// A random [`NodeId`] will be used when publishing each message. The sequence number will be
    /// randomized.
    RandomAuthor,
    /// Message signing is disabled.
    ///
...
impl PublishConfig {
    pub(crate) fn get_own_id(&self) -> Option<&NodeId> {
        match self {
...
pub struct Behaviour<D = IdentityTransform, F = AllowAllSubscriptionFilter> {
    ...
    /// A set of connected peers, indexed by their [`NodeId`] tracking both the [`PeerKind`] and
    /// the set of [`ConnectionId`]s.
    connected_peers: HashMap<NodeId, PeerConnections>,

    /// A map of all connected peers - A map of topic hash to a list of gossipsub peer Ids.
    topic_peers: HashMap<TopicHash, BTreeSet<NodeId>>,

    /// A map of all connected peers to their subscribed topics.
    peer_topics: HashMap<NodeId, BTreeSet<TopicHash>>,
    ...
    fanout: HashMap<TopicHash, BTreeSet<NodeId>>,
    ...
    count_received_ihave: HashMap<NodeId, usize>,

    /// Counts the number of `IWANT` that we sent the each peer since the last heartbeat.
    count_sent_iwant: HashMap<NodeId, usize>,
    ...
    F: TopicSubscriptionFilter + Send + 'static,
{
    ...
    fn check_explicit_peer_connection(&mut self, peer_id: &NodeId) {
    ...
    fn handle_ihave(&mut self, peer_id: &NodeId, ihave_msgs: Vec<(TopicHash, Vec<MessageId>)>) {
            ...
            match subscription.action {
                SubscriptionAction::Subscribe => {
                    ...
                    if !self.explicit_peers.contains(propagation_source)
                        && self.connected_peers.get(propagation_source).is_some()
                    ...
                    application_event.push(Event::Subscribed {
                ...
                SubscriptionAction::Unsubscribe => {
                    ...
                    application_event.push(Event::Unsubscribed {
                        peer_id: *propagation_source,
                        topic: topic_hash.clone(),
                    });
                }
                    ...
                    .map(|(t, peers)| {
                        (
                            ...
                            peers
                                ...
                                .collect::<HashMap<NodeId, f64>>(),
                        )
                    })
                    .collect::<HashMap<TopicHash, HashMap<NodeId, f64>>>()
            })
        }
    ...
    fn forward_msg(
        ...
        msg_id: &MessageId,
        message: RawMessage,
        propagation_source: Option<&NodeId>,
        originating_peers: HashSet<NodeId>,
    ) -> Result<bool, PublishError> {
    ...
    fn control_pool_add(
        control_pool: &mut HashMap<NodeId, Vec<ControlAction>>,
        peer: NodeId,
    ...
    fn accept_connection(&mut self, peer_id: NodeId, endpoint: Connection, is_dialer: bool) {
        ...
        if is_dialer && other_established == 0 && !self.px_peers.contains(&peer_id) {
        ...
        if let Some((peer_score, ..)) = &mut self.peer_score {
            let ip = endpoint.remote_address().ip();
            peer_score.add_ip(&peer_id, ip);
        }
        let connection_id = endpoint.stable_id();

        let (connection_sender, connection_receiver) = mpsc::channel(128);
        let (handler_sender, handler_receiver) = mpsc::channel(128);
        ...
        let connection_task = tokio::task::spawn(async move {
            ...
            let handler = handler;
            // TODO: on connection close, send message to behaviour, to trigger `on_connection_closed`
        });
    ...
    fn on_connection_closed(&mut self, peer_id: NodeId, connection_id: usize, ip: IpAddr) {
        ...
        if remaining_established != 0 {
            ...
            // TODO:
            // if let Some(metrics) = self.metrics.as_mut() {
            //     metrics.peer_protocol_disconnected();
            // }
    ...
    // TODO: add notify address changes from iroh-net
    ...
    async fn run(mut self) {
        ...
        async fn peer_score_interval(
            peer_score: &mut Option<(PeerScore, PeerScoreThresholds, Ticker, GossipPromises)>,
        ) -> Option<&mut PeerScore> {
            match peer_score {
                Some((peer_score, _, interval, _)) => {
                    interval.next().await;
                    Some(peer_score)
                }
                None => None,
